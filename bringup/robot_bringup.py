...
def getCameraResolution():
    ...
    print(camres)
    if camres is not None and camres != '' and camres != 'None':
        ...
        if data is not None:
            if len(data) == 0:
                ...
                if data == '@usbcam':
                    ...
                    tmux.cmd(0, 'roslaunch usbcam_launch ' + imsz)
                elif data[0:8] == '@usbcam_':  # @usbcam_<width>_<height>
                    v = data.split("_")
                    tmux.cmd(0, 'cd %s' % cfolder)
                    tmux.cmd(0, 'roslaunch usbcam_launch ' +
                             'image_width:=%d image_height:=%d' % (int(v[1]), int(v[2])))
                elif data == '@astra':
                    tmux.cmd(0, 'cd %s' % cfolder)
                    tmux.cmd(0, 'roslaunch astra_launch')
                elif data == '@xtion':
                    tmux.cmd(0, 'cd %s' % cfolder)
                    tmux.cmd(0, 'roslaunch xtion2_launch')
                elif data == '@camerakill':
                    tmux.Cc(0)
                elif data == '@videoserver':
                    tmux.cmd(1, 'cd %s' % cfolder)
                    tmux.cmd(1, 'roslaunch videoserver_launch')
                elif data == '@videoserverkill':
                    tmux.Cc(1)
                elif data == '@apriltags':
                    tmux.cmd(2, 'cd %s' % mfolder)
                    tmux.cmd(2, 'roslaunch tags_launch')
