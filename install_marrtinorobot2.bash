...
DEPTH_SENSOR_ARRAY=(realsense  oakd oakdlite oakdpro)
...
function install_oakd {install_oakdlite
...
if [ "$*" == "" ]
    then
        ...
        echo
        echo "Example: $ bash install_marrtinorobot2.bash 2wd rplidar"
        echo "Example: $ bash install_marrtinorobot2.bash 2wd rplidar realsense"
        echo "Example: $ bash install_marrtinorobot2.bash 2wd - realsense"
        echo "Example: $ bash install_marrtinorobot2.bash 2wd"
...
if [[ "$BASE" != "ci" && "$LASER_SENSOR" != "" && "$LASER_SENSOR" != "-" ]] && !(printf '%s\n' "${LASER_SENSOR_ARRAY[@]}" | grep -xq $LASER_SENSOR)
    then
        echo "Invalid marrtinorobot2 laser sensor: $LASER_SENSOR"
        echo
        echo "Valid Options:"
...
if [[ "$BASE" != "ci" && "$DEPTH_SENSOR" != "" ]] && !(printf '%s\n' "${DEPTH_SENSOR_ARRAY[@]}" | grep -xq $DEPTH_SENSOR)
    then
        echo "Invalid marrtinorobot2 depth sensor: $DEPTH_SENSOR"
        echo
        echo "Valid Options:"
...
if [[ "$BASE" != "ci" ]]
    then
        echo
        echo "You are installing marrtinorobot2 on your robot computer."
        echo
        echo "===========SUMMARY============"
        ...
        echo "Create a marrtinorobot2_ws on your $HOME directory."
        echo "Install marrtinorobot2 ROS2 dependencies."
...
if [[ "$BASE" != "ci" ]]
    then
        ### 1. Robot Type
        echo "export marrtinorobot2_BASE=$BASE" >> ~/.bashrc
        ### 2. Sensors
        if [[ "$LASER_SENSOR" != "-" ||  "$LASER_SENSOR" != "" ]]
            then
                echo "export marrtinorobot2_LASER_SENSOR=$LASER_SENSOR" >> ~/.bashrc
        fi

        if [[ "$DEPTH_SENSOR" != "-" ||  "$DEPTH_SENSOR" != "" ]]
            then
            echo "export marrtinorobot2_DEPTH_SENSOR=$DEPTH_SENSOR" >> ~/.bashrc
        fi
        echo
        echo "Do you want to add sourcing of marrtinorobot2_ws on your ~/.bashrc?"
        echo -n "Yes [y] or No [n]: " 
        read reply
        if [[ "$reply" == "y" || "$reply" == "Y" ]]
            then
                echo "source \$HOME/marrtinorobot2_ws/install/setup.bash" >> ~/.bashrc
        else
            echo
            echo "Remember to run $ source ~/marrtinorobot2_ws/install/setup.bash every time you open a terminal."
        fi
fi
